...
export class RoleRoute implements Routes {
  ...
  private initializeRoutes() {
    ...
    this.router.get(`${this.path}`, isAuthenticated, this.role.getRoles);

    // Route to get a specific role by its ID
    this.router.get(`${this.path}/:id`, isAuthenticated, this.role.getRoleById);

    // Route to create a new role, with validation for the incoming data
    this.router.post(`${this.path}`, isAuthenticated, ValidationMiddleware(CreateRoleDto), this.role.createRole);

    // Route to update an existing role by its ID, with validation for the incoming data
    this.router.put(`${this.path}/:id`, isAuthenticated, ValidationMiddleware(UpdateRoleDto, false, true), this.role.updateRole);

    // Route to delete one or more roles by their IDs, with validation for the incoming IDs
    this.router.delete(`${this.path}`, isAuthenticated, ValidationMiddleware(DeleteActionDto), this.role.deleteRole);

    // Route to update multiple roles using a bulk action, with validation for the action data
    this.router.post(`${this.path}/update-action`, isAuthenticated, ValidationMiddleware(UpdateActionDto), this.role.updateAction);
  }
}
